...
| v5.30 | 2026-03-06 | T-104(최종검증/Claude Code): 모델 선택 UI + Genspark WebAI 통합 — ModelSelector.tsx(5모델 칩: Opus 4.6/Sonnet 4.6/Gemini Flash/GPT-5 mini/혼합), /genspark 페이지(iframe→새탭 폴백), Sidebar Genspark AI 메뉴(🌐), 대시보드 Genspark 바로가기 카드, ceo-chat ModelSelector 연동(selectedModel state), ceo_chat.py MODEL_ID_MAP(4종)+req.model override, api.ts sendCeoMessage model 파라미터, Docker build 0에러(19.1s 컴파일), aads-dashboard commit edaa0c8 push 확인, aads-server ceo_chat.py commit e88bd23 push 확인, aads-dashboard-t104+aads-server-t104 컨테이너 재배포, HTTP 200 |
| v5.37 | 2026-03-06 | AADS-114: /ops 운영 현황 대시보드 — 사이드바 운영현황 메뉴(/ops), 파이프라인 건전성 4카드(🟢/📋/⏳/🚨 30초 자동갱신), 지시서 라이프사이클 타임라인 테이블(상태별 색상+running 펄스애니메이션+stalled 빨강배경⚠️+프로젝트/상태/날짜 필터), 교차검증 7항목 카드(queue_stall/bridge_integrity/commit_integrity/cost_tracking/env_trend/manager_response/pipeline_flow, ✅/❌+클릭 모달), 비용 트래커(SVG 바차트 7일+파이차트+모델별 테이블), 서버 환경 트렌드(SVG 라인차트+80% 빨강점선+서비스상태+서버탭 68/211/114), 브릿지 활동 로그(접이식 30건+blocked🚫빨강), api.ts getOpsHealthCheck/getOpsDirectiveLifecycle/getOpsCostSummary/getOpsEnvHistory/getOpsBridgeLog 5메서드 추가, npm build 0에러, Docker 재빌드+재배포, commit 2801051, push 완료, HTTP 200 ✅ |
| v5.35 | 2026-03-06 | AADS-117: claude_exec.sh 에러 3단계 분류(classify_error: SWITCH/WAIT_RETRY/UNKNOWN), pre_check_account(사전 계정 검증), check_switch_cooldown(5분 쿨다운 핑퐁방지), handle_execution_error(재귀 2회 스위칭), switch_account() LAST_SWITCH_TIME 기록; pipeline_monitor.sh 신규(2분마다 정체감시: STALLED/HUNG/BRIDGE_DOWN, 5분 경고TG, 10분 자동복구: auto_trigger재시작/HUNG강제정리/브릿지restart, AADS Context API pipeline_health 보고); 서버 211 cron 등록(/var/spool/cron/root), claudebot crontab 동일 등록; 서버 68/114 SSH 접근 불가(68: Permission denied, 114: Connection refused)로 원격 배포 미완 — 별도 수동 배포 필요 |
...
| v5.32 | 2026-03-06 | AADS-110: context-package 서버 환경 스냅샷 자동 포함 — channels.py get_server_environment(server)(system_memory category=server_environment key=env_{server} 조회), format_runtimes/format_projects/format_databases/format_services 마크다운 포맷터 4종, get_context_package 서버 환경 섹션(런타임/프로젝트디렉터리/DB스키마/서비스상태) 삽입, KST 타임존(timedelta+9h), 응답 server+env_snapshot_at 필드 추가, NTV2_MGR server=114 확인, commit 82a5b79, push 200, Docker rebuild+재배포, HTTP 200 ✅ |
| v5.31 | 2026-03-06 | AADS-108: 서버 환경 실시간 스냅샷 수집기 — collect_env_snapshot.py(5분 경량/30분 전체/이벤트 즉시, AADS Context API + /manager/env_*.json 정적 저장), generate_env_snapshots.py(68서버에서 Context API→3서버 통합 JSON+env_index.json), aads-docker-watcher.service(Docker 이벤트 즉시 갱신), git post-commit hook(commit 시 스냅샷 즉시 갱신), auto_trigger.sh task_completed 이벤트 스냅샷 연동, crontab(5분 경량+30분 전체+5분 generate), 68서버 full snapshot 실행+env_68.json 생성 확인, aads-server commit 4a2d123 push 완료. 환경변수: SERVER_NAME=68, AADS_API_URL=http://localhost:8100/api/v1, PROJECT_DIRS=/root/aads/aads-server,/root/aads/aads-dashboard,/root/aads/aads-docs. 조회URL: https://aads.newtalk.kr/manager/env_68.json |
